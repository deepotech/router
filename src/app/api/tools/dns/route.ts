...
const VALID_TYPES: RecordType[] = ["A", "AAAA", "MX", "TXT", "NS", "CNAME", "SOA", "PTR", "SRV"];
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    switch (type) {
      ...
      case "SOA":   records = await dns.resolveSoa(clean); break;
      case "PTR":   records = await dns.resolvePtr(clean); break;
      case "SRV":   records = await dns.resolveSrv(clean); break;
    }
